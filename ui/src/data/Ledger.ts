...
export const pengerInn = (
  chartLabels: number[],
  ledger: Array<LedgerRow>
): Array<number> => {
  return chartLabels.map(
    (l) =>
      ledger.find((i) => i.dayOfMonth === l && i.accountTo === "user")
        ?.amount || 0
  );
};

export const pengerInnTotal = (
  chartLabels: number[],
  ledger: Array<LedgerRow>
): number => {
  return pengerInn(chartLabels, ledger).reduce(
    (total, amount) => total + amount
  );
};

export const pengerUt = (
  chartLabels: number[],
  ledger: Array<LedgerRow>
): Array<number> => {
...
export const pengerUtTotal = (
  chartLabels: number[],
  ledger: Array<LedgerRow>
): number => {
  return pengerUt(chartLabels, ledger).reduce(
    (total, amount) => total + amount
  ); // * -1
};
